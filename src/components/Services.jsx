...
          </div>
<h2 className="text-6xl font-bold text-center mb-12 flex ml-10 text-white" style={{fontFamily: "Unbounded Placeholder, sans-serif"}}>What we are Offering </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {services.map((service, idx) => (
          ...
          style={{ backgroundColor: "rgba(31, 29, 29,1)" }}>
            <div>
              <div className="text-2xl mb-4 text-orange-400">{service.icon}</div>
              <h3 className="text-2xl font-semibold mb-2">{service.title}</h3>
              <p className="text-gray-500 text-sm">{service.desc}</p>
            </div>
            <button className="mt-6 text-[#fff] font-semibold flex items-center gap-1 hover:underline">
              LEARN MORE 
            </button>
          </div>
        ))}
